...
class Image
{
  public:
    ...
    static std::expected<std::unique_ptr<Image>, util::Error>
    fromPalettedData(const std::vector<uint8_t>& palettedData, uint32_t width, uint32_t height,
                     const Palette& palette);

    /// Create from pre-existing RGBA data (4 bytes per pixel)
    static std::expected<std::unique_ptr<Image>, util::Error>
    fromRGBAData(const std::vector<uint8_t>& rgbaData, uint32_t width, uint32_t height);

...
bool collideWithLine(const Vector3& p1, const Vector3& p2, const Vector3& spherePos,
                     const Vector3& dir, float radius, float currentDist, float& newDist,
                     Vector3& intersectionPoint)
{
    ...
    float b = edgeLengthSq * (2.0f * dir.dot(sphereToVertex)) -
              (2.0f * edgeDotDir * edgeDotSphereToVertex);
    float c = edgeLengthSq * (radius * radius - sphereToVertexLengthSq) +
...
bool containsPoint(const formats::ParsedFace& face, const VertexList& vertices,
                   const Vector3& point)
{
    ...
    for (size_t i = 0, j = numVerts - 1; i < numVerts; j = i++)
    {
        ...
        if (axis == 0)
        {
            vxi = static_cast<float>(vi.y);
            vyi = static_cast<float>(vi.z);
            vxj = static_cast<float>(vj.y);
            vyj = static_cast<float>(vj.z);
            px = point.y;
            py = point.z;
        }
        else if (axis == 1)
        {
            vxi = static_cast<float>(vi.x);
            vyi = static_cast<float>(vi.z);
            vxj = static_cast<float>(vj.x);
            vyj = static_cast<float>(vj.z);
            px = point.x;
            py = point.z;
        }
        ...
        {
            vxi = static_cast<float>(vi.x);
            vyi = static_cast<float>(vi.y);
            vxj = static_cast<float>(vj.x);
            vyj = static_cast<float>(vj.y);
            px = point.x;
            py = point.y;
        }
                           ...
                           float& moveDistance, Vector3& collisionPoint)
{
    ...
    float centerFaceDist =
        pos.dot({face.normalFX, face.normalFY, face.normalFZ}) - face.normalFDist;
    float distToPlane = (centerFaceDist - radius) / -dirNormalDot;

    if (distToPlane < -radius)
        return false;

    if (distToPlane > 65536.0f)
        return false;

    Vector3 projectedPos =
        pos + dir * distToPlane - Vector3{face.normalFX, face.normalFY, face.normalFZ} * radius;
...
template <typename VertexList>
void processFaces(CollisionState& state, const std::vector<formats::ParsedFace>& faces,
                  const VertexList& vertices)
{
    for (const auto& face : faces)
    {
        ...
        float minZ =
            std::min(state.positionLo.z - state.radiusLo, state.positionHi.z - state.radiusHi);
        float maxZ =
            std::max(state.positionLo.z + state.radiusLo, state.positionHi.z + state.radiusHi);
        ...
        if (maxX < face.minX || minX > face.maxX || maxY < face.minY || minY > face.maxY ||
            maxZ < face.minZ || minZ > face.maxZ)
            continue;

        float moveDist = state.adjustedMoveDistance;
        Vector3 colPos;
        if (collideSphereWithFace(face, vertices, state.positionLo, state.radiusLo, state.direction,
                                  moveDist, colPos))
        {
        ...
        if (collideSphereWithFace(face, vertices, state.positionHi, state.radiusHi, state.direction,
                                  moveDist, colPos))
        {
            if (moveDist > ALLOWED_OVERSHOOT)
            {
                        ...
                        const formats::ODMMapData* odm, float deltaMs, const PhysicsConfig& config)
{
    ...
    for (int attempt = 0; attempt < 5; ++attempt)
    {
        ...
        if (state.moveDistance <= MIN_MOVE_DISTANCE)
            break;
        ...
        Vector3 adjustedPos =
            pos + state.direction * std::max(0.0f, state.adjustedMoveDistance - CLOSEST_DIST);
        pos = adjustedPos;
        ...
        else if (state.hit.type == HitType::Face)
        {
            ...
            Vector3 slidePlaneNormal =
                adjustedPos + Vector3{0, 0, state.radiusLo} - slidePlaneOrigin;

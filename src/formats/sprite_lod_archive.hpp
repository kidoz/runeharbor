 ...
 * - 12-byte sprite names (vs 4-byte + extension marker)
...
{
    char name[12];        // Sprite name (up to 12 chars, null-padded)
    uint32_t attributes;  // Sprite attributes/flags
...
struct SpriteFileHeader
{
    ...
    uint32_t totalSize;        // Total data size
    ...
    // Followed by line offset table (height * 2 bytes, 0xFFFF = empty line)

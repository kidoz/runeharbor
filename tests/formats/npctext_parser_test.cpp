...
Lord Markham has supplemented the normally sparse village with a weapon, armor, and alchemist shop for your convenience during the contest.  A temple and basic magical guilds are also available.  I would suggest making sure that you are completely ready before you start off on the contest.)###");
    ...
    SECTION("Second entry is correct (with Notes, empty Owner)")
    {
        ...
        REQUIRE(entries[1].text == "I respect your decision.  I am, however, a patient man, and will offer you this chance until a winner in the contest is proclaimed.");
    ...
    SECTION("Third entry is correct (with Notes and Owner)")
    {
        ...
        std::string extended_sample_data = sample_data + R"(6	"If you win, you'll be in charge of one of the most scenic areas in all Erathia!  Harmondale is just outside of the Tularean Forest, right on the edge of the Elf-Human border.  And I'm sure you'll love the castle.  It's a bit of a fixer-upper, but it's quite roomy and has excellent ventilation.  It breaks my heart to part with this property, but I feel that the time has come for me to give something back to the people."	speech stays until contest is won	Markham
)";
        std::vector<uint8_t> extended_data(extended_sample_data.begin(), extended_sample_data.end());
        
        parser.parse(extended_data);
        const auto& entries = parser.getNPCTextEntries();
        REQUIRE(entries.size() == 4); // 3 original + 1 appended
        REQUIRE(entries[3].id == 6);
        REQUIRE(entries[3].text == "If you win, you'll be in charge of one of the most scenic areas in all Erathia!  Harmondale is just outside of the Tularean Forest, right on the edge of the Elf-Human border.  And I'm sure you'll love the castle.  It's a bit of a fixer-upper, but it's quite roomy and has excellent ventilation.  It breaks my heart to part with this property, but I feel that the time has come for me to give something back to the people.");
...
2	text2	Note2
        ...
        REQUIRE(parser.getNPCTextEntries().size() == 2); // 2 entries parsed (malformed line skipped)
        REQUIRE(parser.getNPCTextEntries()[0].id == 1);
